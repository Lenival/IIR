...
N_0 = 0.01;                                 % Densidade espectral do ruído
...
plot(f,abs(Y_b),'m');title('Sinal após o filtro Butterworth');xlim([0 Fs/2]);xlabel('f (Hz)');ylabel('Y_{Butter}(\omega)')

subplot(2,2,2)
Y_c1 = fftshift(fft(y_c1));
plot(f,abs(Y_c1),'g');title('Sinal após o filtro Chebyshev I');xlim([0 Fs/2]);xlabel('f (Hz)');ylabel('Y_{ChebyI}(\omega)')

subplot(2,2,3)
Y_c2 = fftshift(fft(y_c2));
plot(f,abs(Y_c2),'c');title('Sinal após o filtro Chebyshev II');xlim([0 Fs/2]);xlabel('f (Hz)');ylabel('Y_{ChebyII}(\omega)')
